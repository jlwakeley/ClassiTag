...
def draw_overlay(
    draw: ImageDraw.ImageDraw, width: int, height: int, classification: str, outline: bool
) -> None:
    ...
    text_position_top = (
        (width - text_width) // 2,
        (top_overlay_height - text_height) // 2 + 1 if outline else 0,
    )
    ...
    # Add black outline
    if outline:
        draw.rectangle([0, 0, width, height], outline=(0, 0, 0), width=2)  # type: ignore # noqa: PGH003


def save_image_with_overlay(
    img: Image.Image, image_path: pathlib.Path, classification: str, outline: bool
) -> None:
    ...
    draw_overlay(draw, width, height + 2 * BAR_HEIGHT, classification, outline)
...
def add_overlay_to_directory(
    directory_path: pathlib.Path, classification: str, outline: bool
) -> None:
    ...
    for file_path in directory_path.glob("*"):
        if file_path.is_file() and file_path.suffix.lower() in {".png", ".jpg", ".jpeg", ".bmp"}:
            ...
            if img:
                save_image_with_overlay(img, file_path, classification, outline)
...
def create_command_line_interface(
    ...
    add_overlay_to_directory(directory_path, classification, outline)
...
def main(directory_path: pathlib.Path, classification: str, outline: bool) -> None:
    """
    ...
    outline = Enable an optional border for documentation contrast.
    """
    create_command_line_interface(directory_path, classification, outline)
